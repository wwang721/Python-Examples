...
#  /tkinter/GetMouse
#
#  Created by WW on 2018/10/1.
#  Copyright © 2018 Wang Wei. All rights reserved.
#
...
def nextRoot4():
	...
	def nextRoot3():
		...
		def xyplot():
			...
			root5 = tk.Tk()
			root5.title("GetMouse")
			
			global xs, ys
			f = Figure(dpi=150)
			a = f.add_subplot(111)
			
			a.plot(xs,ys,'o', color='red', lw=2)
			
			myfont = FontProperties(fname=r"/Library/Fonts/Songti.ttc")
			a.set_title("Y-X散点图", fontproperties=myfont)
			a.set_xlabel("X")
			a.set_ylabel("Y")
			
			canvas = FigureCanvasTkAgg(f, master=root5)
			canvas.show()
			
			canvas.get_tk_widget().pack(side=tk.TOP, fill=tk.BOTH, expand=1)
			#-----------------------------------------------------------------------
			toolbar = NavigationToolbar2TkAgg(canvas, root5)
			toolbar.update()
		...
		global fp, A
		#A = os.path.dirname(os.path.realpath(sys.argv[0]))		#for pyinstaller package
		A = '.'
		...
		tk.Button(root3, text="完成",font=("Songti", 12),  command=xyplot, width=5).pack(side=tk.TOP)
